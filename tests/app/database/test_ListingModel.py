...
def test_crud_listing_model_cascade_to_address(get_test_listing_model, get_test_address_model, get_test_db):
    session = get_test_db
    ...
    session.flush()
    assert results.id and results.id != 0
    ...
    # DELETE and commit, we'll need to clean up test data, delete can be commented out to create test data
    repo.delete(results.id)
    session.commit()
